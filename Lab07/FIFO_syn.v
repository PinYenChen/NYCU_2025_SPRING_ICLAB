...
reg [$clog2(WORDS):0] w_addr, wq2_rptr;
reg [$clog2(WORDS):0] r_addr, rq2_wptr;
...
*/
//gray code
binarytogray w_addr_gray(.binary(w_addr), .gray(wptr));
binarytogray r_addr_gray(.binary(r_addr), .gray(rptr));
...
//empty or full
always @(*) begin
    if (wptr == {!wq2_rptr[6],!wq2_rptr[5], wq2_rptr[4:0]}) begin
        wfull = 1;
    end
    else begin
        wfull = 0;
...
/* this version would delay one cycle
always @(posedge wclk or negedge rst_n) begin
    if (!rst_n) begin
        wfull <= 0;
    end
...
DUAL_64X32X1BM1 u_dual_sram (
    .CKA(wclk),
    .CKB(rclk),
    .WEAN((!winc || wfull)), // don't write : high when winc not valid and sram is full
    .WEBN(1'b1),
    .CSA(1'b1),
    .CSB(1'b1),
    .OEA(1'b1),
